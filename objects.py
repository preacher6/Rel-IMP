...
class Init(pygame.sprite.Sprite):
    def __init__(self, position):
        ...
        self.tipo = 'ini'
        self.image = pygame.image.load(os.path.join('pics', 'ini.png'))
        self.rect = self.image.get_rect()
        self.pos = (position[0]+20, position[1]+20)
    ...
    def draw(self, screen):
        ...
        screen.blit(self.font.render(self.tag, True, COLOR_FONT), (self.pos[0]-10, self.pos[1]+20))
...
class Module(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        for nodo in self.nodos:
            ...
            nodo.draw(screen)
        screen.blit(self.image, self.rect)
        screen.blit(self.font.render(self.tag, True, COLOR_FONT), (self.pos[0], self.pos[1] + 80))


class Caja(pygame.sprite.Sprite):
    def __init__(self, pos, cont, name="", orien=True, draw_box=True):
        pygame.sprite.Sprite.__init__(self)
        self.tipo = 'caja'
        ...
        self.rect.x = pos[0]    
        self.rect.y = pos[1]
        self.tag = name+'Bloque_'+str(cont)
        self.sym = sy.symbols('Bloque_' + str(cont))
        self.id = cont
        self.name = name
...
class Knn(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        self.calc_value()
        screen.blit(self.image, self.rect)
        screen.blit(self.font.render(self.tag, True, COLOR_FONT), (self.pos[0] + 38, self.pos[1] - 20))
...
class Stand(pygame.sprite.Sprite):
    def __init__(self, pos, cont, num_rows=2):
        ...
        for ind in range(num_rows):
            self.cajas.add(Caja((pos[0]+80, pos[1]+self.node_dt*ind), ind+1, name=self.tag+'_', draw_box=False))
        self.nodos = pygame.sprite.Group()
        self.calc_lines()
        self.calc_nodes()
        self.conexiones = pygame.sprite.Group()  # Conexiones anexas al elemento
    ...
    def draw(self, screen):
        self.calc_value()
        screen.blit(self.font.render(self.tag, True, COLOR_FONT), (self.pos[0]-10, self.pos[1]+100))
        screen.blit(self.image, self.rect)
...
class Kdn(pygame.sprite.Sprite):
    def __init__(self, pos, cont, num_rows=2, num_active=1):
        ...
        self.own_syvalye = sy.exp(-(t * float(self.alpha)) ** float(self.betha))
        for ind in range(2):
            self.cajas.add(Caja((pos[0]+60, pos[1]+self.node_dt*ind), ind+1, name=self.tag+'_', draw_box=False))
        self.left = []
        self.right = []
        self.nodos = pygame.sprite.Group()
        self.calc_lines()
    ...
    def draw(self, screen):
        self.calc_value()
        screen.blit(self.image, self.rect)
        screen.blit(self.font.render(self.tag, True, COLOR_FONT), (self.pos[0]+60, self.pos[1] - 20))
        for caja in self.cajas:
            caja.alpha = self.alpha
            caja.betha = self.betha
            caja.mod = self.mod
