...
class Property(pygame.sprite.Sprite):
    def __init__(self, workspace_size=(900, 520)):
        ...
        self.mensaje2 = MessageBox(position=(320, 120), text_position=[15, 20],
                                  button_position=[445, 350], size=[370, 445])  # Objeto para desplegar mensajes
        self.show_msg = False
        self.show_msg2 = False
        self.time_play = False  # Tiempo para play
        self.time_plot = False  # Tiempo para plot
        self.modulos = True  # Indica que se encuentra dentro de dibujar modulos
        self.valid_workspace_knn = pygame.Rect(self.pos_workspace[0], self.pos_workspace[1],
                                               self.SIZE_WORKSPACE[0], self.SIZE_WORKSPACE[1] - 150)

    def init_properties(self):
        self.list_box_systems = ListBox(size=(110, 150), posi=(25, 370), num_rectas=5)
        self.container = Container((self.pos_workspace[0],
                               self.pos_workspace[1] - 30), self.cont, self.tag)  # Primera pestaña
        self.cont_selected = self.container
        self.elementos['containers'].add(self.container)  # Agregar pestaña a lista de pestañas (Dentro de diccionario)
        opciones = ['module', 'play']  # Pestañas de opciones disponibles
        for elem, opcion in enumerate(opciones):
            active = True if elem == 0 else False  # Esta activa la pestaña si es la primera opcion
            position = (self.pos_workspace[0] + 310, self.pos_workspace[1]+470)
            pestana_opcion = OptionPanels(opcion, position, elem, active=active)
            self.elementos['opciones'].add(pestana_opcion)
        self.list_box_systems.add_data(self.container)
        ...
        self.check = CheckButton('Check', 'Rotar', position=(self.pos_proper[0]+15, self.pos_proper[1]+130), color=SEMIWHITE3)
        radio1_out = RadioButton('Radio1', 'Confiabilidad', position=(self.pos_plot[0] + 5, self.pos_plot[1] + 30),
                                 active=True)
        radio2_out = RadioButton('Radio2', 'Medida de Birnbaum', position=(self.pos_plot[0] + 5, self.pos_plot[1] + 65))
        radio3_out = RadioButton('Radio3', 'Potencial de mejora',
                                 position=(self.pos_plot[0] + 5, self.pos_plot[1] + 100))
        radio4_out = RadioButton('Radio4', 'Valor de logro de riesgo',
                                 position=(self.pos_plot[0] + 5, self.pos_plot[1] + 135))
        radio5_out = RadioButton('Radio5', 'Valor de reducción de riesgo',
                                 position=(self.pos_plot[0] + 5, self.pos_plot[1] + 170))
        radio6_out = RadioButton('Radio6', 'Importancia de la criticidad',
                                 position=(self.pos_plot[0] + 5, self.pos_plot[1] + 205))
        self.radio_out = [radio1_out, radio2_out, radio3_out, radio4_out, radio5_out, radio6_out]
    ...
    def draw_containers(self, screen):
        ...
        self.list_box_systems.draw_pestañas(screen)
    ...
    def add_container(self, new=True, old_container=None):
        ...
        if not existe:  # Si es nuevo se crea y se añade al grupo de contenedores
            ...
            self.list_box_systems.add_data(container_new)
    ...
    def delete_container(self, position):
        ...
        container_selected = self.cont_selected
        if len(self.elementos['containers']) > 1:
            for container in self.elementos['containers']:
                if container == container_selected:
                    contador = container.cont
                    self.elementos['containers'].remove(container)
                    self.list_box_systems.del_data(container)
                    if self.list_box_systems.conten_actual > len(self.list_box_systems.list_items):
                        self.list_box_systems.conten_actual = self.list_box_systems.conten_actual - 1
                    print(self.list_box_systems.conten_actual)
                    self.cont -= 1
    ...
    def select_container(self, position, screen):
        """Seleccionar pestaña a activar"""
        container_selected = self.list_box_systems.list_items[self.list_box_systems.conten_actual - 1]
        self.cont_selected = container_selected
        for container in self.elementos['containers']:
            if container == container_selected:
    ...
    def check_actions(self, position):
        ...
        if self.edit_rect.collidepoint(position):  # Información
            self.cancel()
            self.actions = [0] * 9
            self.actions[6] = 1
        if self.info_rect.collidepoint(position):  # Información
            self.cancel()
            self.actions = [0]*9
            self.actions[7] = 1
        if self.help_rect.collidepoint(position):  # Ayuda
            self.cancel()
            self.actions = [0]*9
            self.actions[8] = 1
    ...
    def exec_actions(self, screen, position, abs_position):
        ...
        if self.actions[0]:  # Connect
        ...
        elif self.actions[1]:  # Eliminar conexión
            pygame.mouse.set_visible(False)
            ...
            if self.list_box_modules.accept2.recta.collidepoint(position):
                self.list_box_modules.accept2.over = True
            else:
                self.list_box_modules.accept2.over = False
            ...
            self.show_msg2 = True
            self.mensaje2.text = ["Plataforma para la identificación de componentes críticos", "-REL-Imp-",
                                 " ", "Autores", "", "Mauricio Holguín Londoño", "Andrés Escobar Mejía",
                                  "Carlos Alberto Ramirez Vanegas", "",
                                  "La plataforma permite la identificación de los componentes críticos",
                                  "dentro de un sistema representado por diagramas de bloques (método ",
                                  "gráfico por preferencia para la representación de sistemas físicos ",
                                  "dentro del estudio de confiabilidad), esto mediante diferentes medidas",
                                  "de importancia. REL-Imp facilita entonces el análisis de sistemas",
                                  "simples y complejos mediante la visualización de las diferentes ",
                                  "métricas para cada elemento de un sistema, brindando así la posibilidad",
                                  "de determinar que componentes son más críticos desde un punto de vista",
                                  "del impacto en el sistema de confiabilidad, proporcionando así una",
                                  "perspectiva cuantitativa sobre los contribuyentes dominantes al riesgo y",
                                  "la sensibilidad del riesgo a los cambios en los valores de los insumos.",
                                ...
                                if knn_ind.num_rows < 5:
                                    if knn_ind.tag == self.elem_selected:
                                        ...
                                        if conectado:
                                            ...
                                            position = (caja.pos[0]-self.pos_workspace[0], caja.pos[1]+100-self.pos_workspace[1])
                                            self.valid_workspace_knn = pygame.Rect(self.pos_workspace[0],
                                                                                   self.pos_workspace[1],
                                                                                   self.SIZE_WORKSPACE[0],
                                                                                   self.SIZE_WORKSPACE[1] - 150)
                                            if self.valid_workspace_knn.collidepoint(position):
                                                if not hit:
                                                ...
                                                self.show_msg = True
                                                self.mensaje.text = 'Elemento fuera del área de trabajo'
                                        else:
                                            self.show_msg = True
                                            self.mensaje.text = 'El elemento debe estar desconectado'
                                    ...
                                    if knn_ind.tag == self.elem_selected:
                                        ...
                                        if conectado:
                                            if len(knn_ind.cols[ind]) < 6:
                                                ...
                                                self.valid_workspace_knn = pygame.Rect(self.pos_workspace[0],
                                                                                       self.pos_workspace[1],
                                                                                       self.SIZE_WORKSPACE[0]-140,
                                                                                       self.SIZE_WORKSPACE[1])
                                                print(caja.pos)
                                                print(position)
                                                print(self.valid_workspace_knn)
                                                if self.valid_workspace_knn.collidepoint(position):
                                                    if not hit:
                                                        container.real_items.add(caja)
                ...
                for container in self.elementos['containers']:
                    if container.selected:
                        for stand_ini in container.stand:
                            if self.addline_rect_kdn.collidepoint(push_position):  # Adicionar paralelo
                                if stand_ini.num_elements < 5:
                                    if stand_ini.tag == self.elem_selected:
                                        stand_ini.num_elements += 1

                            if self.reduceline_rect_kdn.collidepoint(push_position):  # Eliminar elemento
    ...
    def move_inside_work(self, screen, position, elemento, space):
        """Funcion para definir la manera en que se ubica el elemento dependiendo de donde se encuentre"""
    ...
    def delete_element(self, position):  # Elimina el elemento seleccionado
        for container in self.elementos['containers']:
            if container.selected:
                ...
                for modulo in container.module:
                    if modulo.rect.collidepoint(position):
                        container.all_module -= 1
                        container.list_box.del_data(modulo)
                        for nodo in modulo.nodos:
                            for key in container.keys:  # Remover elemento de lista de conexiones nodales
                                if nodo in container.nodos_sistema[key]:
                                    container.nodos_sistema[key].remove(nodo)
                            container.nodos.remove(nodo)  # Remover nodo de lista de nodos del contenedor
                        container.module.remove(modulo)
                        container.own_items.remove(modulo.tag)
                        container.items.remove(modulo)
                        container.real_items.remove(modulo)
    ...
    def text_boxes(self):
        ...
        self.time_field = TextBox((150, 260, 100, 20), acepted=ACCEPTED_NUM, id="name_con", active=True,
                                  clear_on_enter=False, inactive_on_enter=True)
        self.time_field_plot = TextBox((150, 480, 100, 20), acepted=ACCEPTED_NUM, id="name_con", active=True,
                                 clear_on_enter=False, inactive_on_enter=True)
        self.time_field_play = TextBox((250, 500, 100, 20), acepted=ACCEPTED_NUM, id="name_con", active=True,
                                 clear_on_enter=False, inactive_on_enter=True)
    ...
    def check_text(self, event):
        ...
        if self.time_play:
            self.time_field_play.get_event(event)
        if self.time_plot:
            self.time_field_plot.get_event(event)
    ...
    def draw_selected(self, screen, position, pushed):
        ...
        if self.knn_mini_rect.collidepoint(pushed) or self.hold_knn:
            self.cancel()
            self.hold_knn = True
            valid_workspace = pygame.Rect(self.pos_workspace[0], self.pos_workspace[1],
                                          self.SIZE_WORKSPACE[0] - 200, self.SIZE_WORKSPACE[1] - 180)
            ...
            self.title = 'knn'
            self.draw_inside_work(screen, position, elemento, valid_workspace)
        if self.stand_mini_rect.collidepoint(pushed) or self.hold_stand:
            self.cancel()
            ...
            self.title = 'stand'
            self.draw_inside_work(screen, position, elemento, valid_workspace)
        if self.kdn_mini_rect.collidepoint(pushed) or self.hold_kdn:
            self.cancel()
            self.hold_kdn = True
            valid_workspace = pygame.Rect(self.pos_workspace[0], self.pos_workspace[1],
                                          self.SIZE_WORKSPACE[0] - 200, self.SIZE_WORKSPACE[1] - 180)
        ...
        if self.ini_mini_rect.collidepoint(pushed) or self.hold_ini:
            self.cancel()
            self.hold_ini = True
            valid_workspace = pygame.Rect(self.pos_workspace[0], self.pos_workspace[1],
                                          self.SIZE_WORKSPACE[0] - 200, self.SIZE_WORKSPACE[1] - 80)
            elemento = self.ini
            self.title = 'ini'
            self.draw_inside_work(screen, position, elemento, valid_workspace)

        if self.end_mini_rect.collidepoint(pushed) or self.hold_fin:
            self.cancel()
            self.hold_fin = True
            valid_workspace = pygame.Rect(self.pos_workspace[0], self.pos_workspace[1],
                                          self.SIZE_WORKSPACE[0] - 60, self.SIZE_WORKSPACE[1] - 80)
            elemento = self.fin
            self.title = 'fin'
            self.draw_inside_work(screen, position, elemento, valid_workspace)


        if self.draw_module or self.hold_module:
    ...
    def put_element(self):
        ...
        if self.hold_caja:
            ...
            for container in self.elementos['containers']:
                if container.selected:
                    if not "Inicio" in container.own_items:
                        inicio = Init(self.put_position)
                        container.limites.add(inicio)
                        container.nodos.add(inicio.nodos)
                        container.own_items.append(inicio.tag)
                        container.items.add(inicio)
                        container.real_items.add(inicio)
                    else:
                        print('solo 1 inicio')
        elif self.hold_fin:
            print('in')
            for container in self.elementos['containers']:
                if container.selected:
                    if not "Fin" in container.own_items:
                        fin = End(self.put_position)
                        container.limites.add(fin)
                        container.nodos.add(fin.nodos)
                        container.own_items.append(fin.tag)
                        container.items.add(fin)
                        container.real_items.add(fin)
                    else:
                        print('solo 1 inicio')
            ...
            for container in self.elementos['containers']:
                if container.selected:
                    if self.elem_modulo.name not in container.own_items:
                        container.all_module += 1
                        ...
                        self.show_msg = True
                        self.mensaje.text = 'Subsistema ya se encuentra dentro de la hoja de trabajo'
    ...
    def draw_on_screen(self, screen, position, push_position):  # Aca se dibujan la mayoria de acciones realizadas
        ...
        for elemento in self.elementos['opciones']:
            if elemento.name == 'module':
                                                ...
                                                for caja in container.cajas:
                                                    if caja.tag == self.elem_selected:
                                                        if nombre and buffer1 and buffer2:
                                                            ...
                                                            container.own_items.remove(self.elem_selected)
                                                            caja.tag = "".join(self.name_element.buffer)
                                                            container.own_items.append(nombre)
                                                            caja.alpha = "".join(self.box_field1.buffer)
                                                            caja.betha = "".join(self.box_field2.buffer)
                                                            self.elem_proper = False
                                                            self.text_status = [0]*5
                                                            for index, nodo in enumerate(caja.nodos):
                                                                nodo.tag = 'Nodo_' + str(index+1) + caja.tag
                                                                nodo.name_element = caja.tag
                                                        else:
                                                            self.show_msg = True
                                                ...
                                                for knn_ind in container.knn:
                                                    if knn_ind.tag == self.elem_selected:
                                                        if nombre:  # Verificar si el nombre no está vacío
                                                            ...
                                                            container.own_items.remove(self.elem_selected)
                                                            container.own_items.append(nombre)
                                                            for index, nodo in enumerate(knn_ind.nodos):
                                                                nodo.tag = 'Nodo_' + str(index+1) + knn_ind.tag
                                                                nodo.name_element = knn_ind.tag
                                                ...
                                                for stand_ind in container.stand:
                                                    if stand_ind.tag == self.elem_selected:
                                                        if nombre and buffer1 and buffer2:
                                                            container.own_items.remove(self.elem_selected)
                                                            container.own_items.append(nombre)
                                                            stand_ind.tag = "".join(self.name_element.buffer)
                                                            stand_ind.alpha = "".join(self.box_field1.buffer)
                                                            stand_ind.betha = "".join(self.box_field2.buffer)
                                                            for index, nodo in enumerate(stand_ind.nodos):
                                                                nodo.tag = 'Nodo_' + str(index+1) + stand_ind.tag
                                                                nodo.name_element = stand_ind.tag
                                                            self.elem_proper = False
                                                            self.text_status = [0] * 5
                                                        else:
                                                            self.show_msg = True
                                                ...
                                                for kdn in container.kdn:
                                                    if kdn.tag == self.elem_selected:
                                                        if nombre and buffer1 and buffer2:
                                                            ...
                                                            for index, nodo in enumerate(kdn.nodos):
                                                                nodo.tag = 'Nodo_' + str(index+1) + kdn.tag
                                                                nodo.name_element = kdn.tag
                                                            self.elem_proper = False
                                                            self.text_status = [0] * 5
                                                        else:
                                                            self.show_msg = True
                                ...
                                for knn in container.knn:
                                    if knn.tag == self.elem_selected:
                                        if nombre:
                                            ...
                                            container.own_items.remove(self.elem_selected)
                                            container.own_items.append(nombre)
                                            for index, nodo in enumerate(knn.nodos):
                                                nodo.tag = 'Nodo_' + str(index + 1) + knn.tag
                                                nodo.name_element = knn.tag
                                            self.elem_proper = False
                                            self.text_status = [0] * 5
                                            for set_box in knn.cols:
                                                for caja in set_box:
                                                    caja.tag = caja.tag.replace(old_name, knn.tag)
                                        else:
                                            self.show_msg = True
                                            self.mensaje.text = 'Todos los campos deben estar llenos'
                if self.elem_type:
                ...
                if elemento.active:
                    self.modulos = False  # Tiempo para play
                    self.time_play = False
                    self.time_plot = True
                    for container in self.elementos['containers']:
                        if container.selected:
                    ...
                    if self.time:
                        for container in self.elementos['containers']:
                            if container.selected:
                                self.time_field_plot.buffer = [char for char in str(container.time_plot)]
                    self.time = False
                    self.time_field_plot.update()
                    self.time_field_plot.draw(screen)
                    screen.blit(self.font.render('Tiempo: ', True, (0, 0, 0)), (90, 480))
                    screen.blit(self.ok_n, self.ok_rect4)
                    if self.ok_rect4.collidepoint(position):  # Mouse over button
                        screen.blit(self.ok_s, self.ok_rect4)
                    if self.ok_rect4.collidepoint(push_position):  # Boton presionado
                        for container in self.elementos['containers']:
                            if container.selected:
                                buffer_time = "".join(self.time_field_plot.buffer)
                                if buffer_time:
                                    if buffer_time[0] != '0':
                                        container.time_plot = buffer_time
                                        container.time_plot = int(container.time_plot)
                                        self.time = True
                                    else:
                                        self.show_msg = True
                                        self.mensaje.text = 'Formato inválido'
                                else:
                                    self.show_msg = True
                                    self.mensaje.text = 'Formato inválido'
                ...
                if elemento.active:
                    ...
                    for container in self.elementos['containers']:
                        if container.selected:
                            if container.all_connected:
                                ...
                                screen.blit(self.canvas_space(self.fig), (380, 150))
                                t = container.time
                                ...
                                for radio in self.radio_out:
                                    ...
                                    if len(container.items) > 3:
                                        if radio.recta.collidepoint(push_position):
                                            ...
                                            if radio.name == 'Radio1':
                                                container.type_plot = 1
                                            elif radio.name == 'Radio2':
                                                container.type_plot = 2
                                            elif radio.name == 'Radio3':
                                                container.type_plot = 3
                                            elif radio.name == 'Radio4':
                                                container.type_plot = 4
                                            elif radio.name == 'Radio5':
                                                container.type_plot = 5
                                            elif radio.name == 'Radio6':
                                                container.type_plot = 6
                                            else:
                    ...
                    if self.time:
                        for container in self.elementos['containers']:
                            if container.selected:
                                self.time_field_play.buffer = [char for char in str(container.time)]
                    self.time = False
                    for container in self.elementos['containers']:
                        if container.selected:
                            self.time_field_play.update()
                            self.time_field_play.draw(screen)
                    screen.blit(self.font.render('Tiempo: ', True, (0, 0, 0)), (190, 500))
                    screen.blit(self.ok_n, self.ok_rect3)
                    if self.ok_rect3.collidepoint(position):  # Mouse over button
                        screen.blit(self.ok_s, self.ok_rect3)
                    if self.ok_rect3.collidepoint(push_position):  # Boton presionado
                        for container in self.elementos['containers']:
                            if container.selected:
                                buffer_time = "".join(self.time_field_play.buffer)
                                if buffer_time:
                                    if buffer_time[0] != '0':
                                        if (isinstance(eval(buffer_time), float) or isinstance(eval(buffer_time), int)):
                                            container.time = buffer_time
            ...
            """elif elemento.name == 'config':    PROPENSO A ELIMINARSE
                if elemento.active:
                    self.time_play = False
                    for container in self.elementos['containers']:
                        if container.selected:
    ...
    def surfaces(self):
        ...
        self.pos_button_action = (35, 130)  # Inicio de posiciones acciones
        self.button_act_panel = pygame.Surface((102, 190))  # Superficie para las acciones
        self.button_act_panel.fill(WHITE)
        self.pos_button_elements = (200, 20)  # Inicio de posiciones acciones
        self.button_ele_panel = pygame.Surface((340, 65))  # Superficie para las acciones
        ...
        self.rename_panel = pygame.Surface((200, 110))  # Superficie para las acciones
        self.rename_panel.fill(SEMIWHITE3)
        self.workspace = pygame.Surface(self.SIZE_WORKSPACE)
        self.workspace.fill(WHITE)
        self.pos_workspace = (180, 110)  # inicio espacio de trabajo
        self.proper_panel = pygame.Surface((200, 160))
        self.proper_panel.fill(SEMIWHITE3)
        self.pos_proper = (400, 180)
        self.proper_panel_knn = pygame.Surface((200, 240))
        self.proper_panel_knn.fill(SEMIWHITE3)
        self.pos_proper = (400, 180)
    ...
    def draw_rects(self, screen):
        self.recta_acciones = pygame.Rect((self.pos_button_action), (102, 190))
        pygame.draw.rect(screen, BLACK, (self.recta_acciones), 2)
        self.recta_workspace = pygame.Rect((self.pos_workspace), self.SIZE_WORKSPACE)
        pygame.draw.rect(screen, BLACK, (self.recta_workspace), 2)
        self.recta_elementos= pygame.Rect((self.pos_button_elements), (340, 65))
        pygame.draw.rect(screen, BLACK, (self.recta_elementos), 2)
        self.recta_sistemas = pygame.Rect(self.list_box_systems.posi_container,
                                          (self.list_box_systems.size[0]+self.list_box_systems.scroll.width, self.list_box_systems.size[1]))
        pygame.draw.rect(screen, BLACK, (self.recta_sistemas), 2)
    ...
    def proper_surface(self, screen, position):
        ...
        if self.type_element == 1 or self.type_element == 4 or self.type_element == 3:  # Aca se identifica si el panel es para caja, knn o standby
        ...
        if self.type_element == 1:  # Si es tipo 1, o sea solo caja
        ...
        elif self.type_element == 2:  # Paralelo
            for container in self.elementos['containers']:
                if container.selected:
                    for knn_ind in container.knn:
                        if knn_ind.tag == self.elem_selected:
            ...
            screen.blit(self.font.render('Betha: ', True, (0, 0, 0)),
                        (self.pos_proper[0] + 25, self.pos_proper[1] + 100))
            for container in self.elementos['containers']:
                for stand_in in container.stand:
                    if stand_in.tag == self.elem_selected:
                        if stand_in.mod == 'exp':
                            self.text_status = [0, 1, 0, 0, 0]
                        elif stand_in.mod == 'ray':
                            self.text_status = [0, 0, 1, 0, 0]
                        elif stand_in.mod == 'wei':
                            self.text_status = [0, 0, 0, 1, 0]
    ...
    def type_surface(self, screen, position, pushed):
        ...
        if self.type_element == 1:  # Propiedades para caja sola
                ...
                if textbutton.recta.collidepoint(position):  # Si se pasa sobr eel boton
                    ...
                    if textbutton.recta.collidepoint(pushed):  # Se presiona el boton
                        for container in self.elementos['containers']:  # Buscar contenedor actual
                            if container.selected:
                                for stand in container.stand:
                                    if stand.tag == self.elem_selected:
                                        stand.mod = textbutton.name
                else:
                    textbutton.over = False
            num_elements = 1
            for container in self.elementos['containers']:
            ...
            screen.blit(self.font.render('Nro. elementos:  ' + str(num_elements), True, (0, 0, 0)),
                        (self.pos_proper[0] + 10, self.pos_proper[1] + 130))
    ...
    def name_surface(self, screen, position):
        screen.blit(self.rename_panel, self.pos_rename)
        screen.blit(self.close, self.close_name_rect)
        screen.blit(self.font.render('Nombre  sistema:', True, (0, 0, 0)),
    ...
    def load_pics(self):
        ...
        self.help_s = pygame.image.load(os.path.join('icons', 'help_s.png'))
        self.add_n = pygame.image.load(os.path.join('icons', 'add_n.png'))
        self.add_s = pygame.image.load(os.path.join('icons', 'add_s.png'))
        self.min_n = pygame.image.load(os.path.join('icons', 'del_n.png'))
        self.min_s = pygame.image.load(os.path.join('icons', 'del_s.png'))
        self.edit_n = pygame.image.load(os.path.join('icons', 'edit_n.png'))
        self.edit_s = pygame.image.load(os.path.join('icons', 'edit_s.png'))
        # Elementos
        ...
        self.ini_mini = pygame.image.load(os.path.join('pics', 'ini_mini.png'))
        self.end_mini = pygame.image.load(os.path.join('pics', 'end_mini.png'))
        # Normales
        ...
        self.ini = pygame.image.load(os.path.join('pics', 'ini.png'))
        self.fin = pygame.image.load(os.path.join('pics', 'end.png'))
    ...
    def rect_actions(self, pos_actions):
        ...
        self.connect_rect.x = pos_actions[0]+6
        self.connect_rect.y = pos_actions[1]+4
        self.disconnect_rect = self.disconnect_n.get_rect()
        self.disconnect_rect.x = pos_actions[0] + 56
        self.disconnect_rect.y = pos_actions[1] + 4
        self.move_rect = self.move_n.get_rect()
        self.move_rect.x = pos_actions[0]+6
        self.move_rect.y = pos_actions[1]+48
        self.delete_line_rect = self.delete_line.get_rect()
        # 2da fila
        self.delete_rect = self.delete_n.get_rect()
        self.delete_rect.x = pos_actions[0] + 56
        self.delete_rect.y = pos_actions[1] + 48
        self.export_rect = self.export_n.get_rect()
        self.export_rect.x = pos_actions[0] + 6
        self.export_rect.y = pos_actions[1] + 95
        self.import_rect = self.import_n.get_rect()
        self.import_rect.x = pos_actions[0] + 56
        self.import_rect.y = pos_actions[1] + 95
        # 3ra fila
        ...
        self.ok_rect.y = self.pos_rename[1] + 70
        ...
        self.ok_rect3 = self.ok_n.get_rect()  # Ok para timeplay
        self.ok_rect3.x = self.pos_config[0] + 260
        self.ok_rect3.y = self.pos_config[1] + 285
        self.ok_rect4 = self.ok_n.get_rect()  # Ok para timeplot
        self.ok_rect4.x = self.pos_config[0] + 160
        self.ok_rect4.y = self.pos_config[1] + 265
        ...
        self.info_rect.x = pos_actions[0] + 6
        self.info_rect.y = pos_actions[1] + 145
        self.help_rect = self.help_n.get_rect()
        self.help_rect.x = pos_actions[0] + 56
        self.help_rect.y = pos_actions[1] + 145
        self.add_rect = self.add_n.get_rect()
        self.add_rect.x = self.list_box_systems.posi_container[0] + 10
        self.add_rect.y = self.list_box_systems.posi_container[1] + self.list_box_systems.size[1] + 10
        self.min_rect = self.min_n.get_rect()
        self.min_rect.x = self.list_box_systems.posi_container[0] + 50
        self.min_rect.y = self.list_box_systems.posi_container[1] + self.list_box_systems.size[1] + 10
        self.edit_rect = self.edit_n.get_rect()
        self.edit_rect.x = self.list_box_systems.posi_container[0]+ 90
        self.edit_rect.y = self.list_box_systems.posi_container[1] + self.list_box_systems.size[1] + 10
    ...
    def draw_actions(self, screen, position):
        ...
        if self.add_rect.collidepoint(position):
            screen.blit(self.add_s, self.add_rect)
            screen.blit(self.font.render('Agregar sistema', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))
        else:
            screen.blit(self.add_n, self.add_rect)

        if self.min_rect.collidepoint(position):
            screen.blit(self.min_s, self.min_rect)
            screen.blit(self.font.render('Eliminar sistema', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))
        else:
            screen.blit(self.min_n, self.min_rect)

        if self.edit_rect.collidepoint(position):
            screen.blit(self.edit_s, self.edit_rect)
            screen.blit(self.font.render('Editar nombre de sistema', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))
        else:
            screen.blit(self.edit_n, self.edit_rect)
        ...
        if self.stand_mini_rect.collidepoint(position):
            screen.blit(self.font.render('StandBy', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))
        ...
        if self.ini_mini_rect.collidepoint(position):
            screen.blit(self.font.render('Inicio', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))

        if self.end_mini_rect.collidepoint(position):
            screen.blit(self.font.render('Final', True, (0, 0, 0)),
                        (position[0] + 8, position[1] + 8))

    def rect_elements(self, position):
        self.caja_mini_rect = self.caja_mini.get_rect()
        self.caja_mini_rect.x = position[0] + 20
        self.caja_mini_rect.y = position[1] + 15
        self.knn_mini_rect = self.knn_mini.get_rect()
        self.knn_mini_rect.x = position[0] + 75
        self.knn_mini_rect.y = position[1] + 10
        self.stand_mini_rect = self.stand_mini.get_rect()
        self.stand_mini_rect.x = position[0] + 125
        self.stand_mini_rect.y = position[1] + 10
        self.kdn_mini_rect = self.kdn_mini.get_rect()
        self.kdn_mini_rect.x = position[0] + 195
        self.kdn_mini_rect.y = position[1] + 10
        self.ini_mini_rect = self.ini_mini.get_rect()
        self.ini_mini_rect.x = position[0] + 250
        self.ini_mini_rect.y = position[1] + 15
        self.end_mini_rect = self.end_mini.get_rect()
        self.end_mini_rect.x = position[0] + 300
        self.end_mini_rect.y = position[1] + 15

    def draw_elements(self, screen):
        """Dibujar elementos a seleccionar (caja-mini, stand-by y knn)"""
        screen.blit(self.ini_mini, self.ini_mini_rect)
        ...
        screen.blit(self.end_mini, self.end_mini_rect)

    @staticmethod
    def canvas_space(fig):
        """Dibujar axis sobre la GUI"""
        canvas = backends.backend_agg.FigureCanvasAgg(fig)
        canvas.draw()
