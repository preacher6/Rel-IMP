...
class OptionPanels(pygame.sprite.Sprite):
    ...
    def __init__(self, name, position, cont, active=False, content=None):
        ...
        self.rect.x = position[0]+cont*60
    ...
    def load_pics(self):
        ...
        self.option_s = pygame.image.load(os.path.join('pics', 'fondo_s.png'))
        self.option_n = pygame.image.load(os.path.join('pics', 'fondo_n.png'))
    ...
    def draw_option(self, screen):
        ...
        screen.blit(self.icon, (self.rect[0]+3, self.rect[1]+3))
...
class Container(pygame.sprite.Sprite):
    def __init__(self, pos, cont, tag, content=None):
        ...
        self.time_plot = 30000  # Tiempo para los plots
        self.plot_all = 0
    ...
    def init_containter(self):
        ...
        self.limites.add(Init((self.pos_elem[0]+40, self.pos_elem[1]+221)))
        self.limites.add(End((self.pos_elem[0]+760, self.pos_elem[1]+221)))
    ...
    def build_matriz(self):
        ...
        if irreduced:
            ...
            dicto_end = dicto_final.copy()
            self.func_sym = self.funco.subs(dicto_final)
            derivates = 0
            self.dicto_sol = {}
            for elem, value in zip(variables_sym, values_sym):
                derivates = sy.diff(func_sym, elem)
                dicto_final[elem] = 0
                own_func_0 = func_sym.subs(dicto_final)
                dicto_final = dicto_end.copy()
                dicto_final[elem] = 1
                own_func_1 = func_sym.subs(dicto_final)
                dicto_final = dicto_end.copy()
                self.dicto_sol[elem] = [derivates.subs(dicto_variables), value, own_func_0, own_func_1]
        else:
            self.correct = True
            ...
            for elem, value in zip(variables_sym, values_sym):
                ...
                dicto_final = dicto_end.copy()
                self.dicto_sol[elem] = [derivates.subs(dicto_variables), value, own_func_0, own_func_1]
                x = (derivates.subs(dicto_variables)*(1-value))/(1-self.func_sym)
                t = sy.symbols('t')
                x_vals = np.linspace(0, int(self.time), 500)
                lam_x = (sy.lambdify(t, x, modules=['numpy']))
                y_vals = lam_x(x_vals)
        ...
        if self.type_plot == 1:  # Confiabilidad
            ...
            if len(self.items)>3:
                ...
                for index, element in self.dicto_sol.items():
                    lam_x = sy.lambdify(t, element[0]*(1-element[1]), modules=['numpy'])
                    y_vals = lam_x(x_vals)
                    if isinstance(y_vals, int):
                        y_vals = [y_vals]*500
                    plt.plot(x_vals, y_vals, label=index)
                    plt.legend()
        elif self.type_plot == 4:  # Logro de riesgo
            if len(self.items)>3:
                t = sy.symbols('t')
                x_vals = np.linspace(0, int(self.time), 500)
                plt.style.use('seaborn')  # pretty matplotlib plots
                ...
                for index, element in self.dicto_sol.items():
                    print(element[2])
                    lam_x = sy.lambdify(t, (1-element[2])/(1-self.func_sym), modules=['numpy'])
                    y_vals = lam_x(x_vals[1:])
                    if isinstance(y_vals, int):
                        y_vals = [y_vals]*499
                    plt.plot(x_vals[1:], y_vals, label=index)
                    plt.legend()
        elif self.type_plot == 5:  # reducción de riesgo
            if len(self.items)>3:
                t = sy.symbols('t')
                x_vals = np.linspace(0, int(self.time), 500)
                plt.style.use('seaborn')  # pretty matplotlib plots
                ...
                for index, element in self.dicto_sol.items():
                    print(element[3])
                    lam_x = sy.lambdify(t, (1-self.func_sym)/(1-element[3]), modules=['numpy'])
                    y_vals = lam_x(x_vals[1:])
                    if isinstance(y_vals, int):
                        y_vals = [y_vals]*499
                    plt.plot(x_vals[1:], y_vals, label=index)
                    plt.legend()
        elif self.type_plot == 6:  # Critica
            if len(self.items)>3:
                t = sy.symbols('t')
                x_vals = np.linspace(0, int(self.time), 500)
                plt.style.use('seaborn')  # pretty matplotlib plots
                ...
                for index, element in self.dicto_sol.items():
                    ...
                    lam_x = sy.lambdify(t, (element[0]*(1-element[1]))/(1-self.func_sym), modules=['numpy'])
                    y_vals = lam_x(x_vals[1:])
                    if isinstance(y_vals, int):
                        y_vals = [y_vals]*499
                    plt.plot(x_vals[1:], y_vals, label=index)
                    plt.legend()
        elif self.type_plot == 2:  # Medida de importancia
            if len(self.items)>3:
                t = sy.symbols('t')
                x_vals = np.linspace(0, int(self.time), 500)
                plt.style.use('seaborn')  # pretty matplotlib plots
                plt.cla()
                for index, element in self.dicto_sol.items():
                    lam_x = sy.lambdify(t, element[0], modules=['numpy'])
                    y_vals = lam_x(x_vals)
                    if isinstance(y_vals, int):
                        y_vals = [y_vals]*500
                    plt.plot(x_vals, y_vals, label=index)
